...
is
      ...
      for J in Index_256 range 0 .. 63 loop
         ...
         begin
            ...
            pragma Loop_Invariant
              (for all I in Index_256 range 0 .. J => (F (I) in Mont_Range3));
            pragma Loop_Invariant
              (for all I in Index_256 range J + 1 .. 63 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range 64 .. 64 + J => (F (I) in Mont_Range3));
            pragma Loop_Invariant
              (for all I in Index_256 range 64 + J + 1 .. 127 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range 128 .. 128 + J => (F (I) in Mont_Range3));
            pragma Loop_Invariant
              (for all I in Index_256 range 128 + J + 1 .. 191 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range 192 .. 192 + J => (F (I) in Mont_Range3));
            pragma Loop_Invariant
              (for all I in Index_256 range 192 + J + 1 .. 255 => (F (I) = F'Loop_Entry (I)));
         end;
      end loop;
   ...
   begin
      for J in Index_256 range 0 .. 15 loop
         ...
         begin
            ...
            pragma Loop_Invariant
              (for all I in Index_256 range Start .. Start + J => (F (I) in Mont_Range5));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + J + 1 .. Start + 15 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 16 .. Start + 16 + J => (F (I) in Mont_Range5));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 16 + J + 1 .. Start + 31 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 32 .. Start + 32 + J => (F (I) in Mont_Range5));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 32 + J + 1 .. Start + 47 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 48 .. Start + 48 + J => (F (I) in Mont_Range5));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 48 + J + 1 .. 255 => (F (I) = F'Loop_Entry (I)));
         end;
      ...
      for J in Index_256 range 0 .. 3 loop
         ...
         begin
            ...
            pragma Loop_Invariant
              (for all I in Index_256 range 0 .. Start - 1 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start .. Start + J => (F (I) in Mont_Range7));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + J + 1 .. Start + 3 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 4 .. Start + 4 + J => (F (I) in Mont_Range7));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 4 + J + 1 .. Start + 7 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 8 .. Start + 8 + J => (F (I) in Mont_Range7));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 8 + J + 1 .. Start + 11 => (F (I) = F'Loop_Entry (I)));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 12 .. Start + 12 + J => (F (I) in Mont_Range7));
            pragma Loop_Invariant
              (for all I in Index_256 range Start + 12 + J + 1 .. 255 => (F (I) = F'Loop_Entry (I)));
         end;
   ...
   begin
      for J in Index_256 range Start .. Start + 1 loop
         ...
         pragma Loop_Invariant
           ...
           (for all I in Index_256 range Start .. J => F (I) in Mont_Range8);
         pragma Loop_Invariant
            -- Elements J + 1 .. Start + 1 are unchanged
            (for all I in Index_256 range J + 1 .. Start + 1 => F (I) = F'Loop_Entry (I));
         pragma Loop_Invariant
            --  Elements Start + 2 through J + 2 are updated
            (for all I in Index_256 range Start + 2 .. J + 2 => F (I) in Mont_Range8);
         pragma Loop_Invariant
            --  Elements from J + 3 .. 255 are unchanged
            (for all I in Index_256 range J + 3 .. 255 => F (I) = F'Loop_Entry (I));
      end loop;
   ...
   begin
      for J in I32 range 0 .. 63 loop
         NTT_Inner7_Slice (F     => F,
                           ZI    => 64 + Byte (J),
                           Start => J * 4);
         pragma Loop_Invariant (for all I in Index_256 range 0         .. J * 4 + 3 => F (I) in Mont_Range8);
         pragma Loop_Invariant (for all I in Index_256 range J * 4 + 4 .. 255       => F (I) = F'Loop_Entry (I));
      end loop;
   end NTT_Inner7;
   ...
   begin
      ...
      for K in F'Range loop
         ...
         pragma Loop_Invariant (for all I in Index_256 range 0 .. K => F (I) in BRange);
      end loop;
